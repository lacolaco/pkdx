...
- `$REPO_ROOT/box/meta/m1-constraints.md` → **Champions M-A 制約メモ (必読)** — M-A 使用不可アイテム・メガ枠・技制約・コミュニケーション規則 (略称禁止・名前捏造禁止等)。**version=champions を選ぶ場合は絶対にスキップしない**

**Note**: タイプ相性は `$PKDX type-chart` および `$PKDX coverage` で計算可能。type.json の直接読み込みは不要。

**Note**: `box/meta/m1-constraints.md` はユーザーからの反復指摘を蓄積した learned-lessons ドキュメント。過去作品 (SV/USUM/剣盾) の知識で M-A 外のアイテム (こだわりハチマキ/メガネ、いのちのたま、とつげきチョッキ等) を提案してしまうミスを防ぐため、Phase 0 で必ず読むこと。
