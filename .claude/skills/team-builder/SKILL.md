...
まず入力名をそのまま `pkdx query` に渡す。ほとんどのフォーム (`ウォッシュロトム` / `キュウコン（アローラ）` / `ランドロス（れいじゅう）` 等) はこれだけで引ける。見つからなかった時のみ原種経由で探索する:

1. 原種名 (`キュウコン` / `ロトム` / `ランドロス`) で `pkdx query` を実行
2. 返り値の `forms[]` に該当 version で実在するフォームの一意名が入っている (`["キュウコン（アローラ）"]` / `["ウォッシュロトム", ...]` / `["メガガブリアス"]` 等)
3. ユーザー意図に合う名前を選んで再 query

戦闘面で差異のないフォーム (トリミアン毛型・ビビヨン模様・フラベベ花色・Unown 文字・マホイップ flavor 等) は `forms[]` に列挙されない。`のすがた` / `フォルム` / ` Form(e)` は情報量ゼロなので migration 側で剥がしてある。
