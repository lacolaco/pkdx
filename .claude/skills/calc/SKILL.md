...
フォーム違いはタイプ・種族値・特性が異なるのでダメージ計算に影響する (例: ウォッシュロトムは でんき/みず、ヒートロトムは でんき/ほのお)。入力名はまずそのまま `pkdx query` に渡す (`ウォッシュロトム` / `キュウコン（アローラ）` 等は直接引ける)。見つからなかった時のみ原種を引いて `forms[]` から一意名を探してから再 query する。
