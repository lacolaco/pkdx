...
1. ユーザー入力名をそのまま `pkdx query "<名前>" --version "<version>" --format json` に渡す。Migration (`008_form_name_aliases`) により以下は直接引ける:
...
- 該当 version にそのフォームが実在しない場合は `forms[]` からも除外される (例: SV にはテラパゴス未実装、Champions にはランドロス（れいじゅう）未収録 等) — ユーザーにその旨を伝えて別 version を提案する
- メガシンカは form の一種として `forms[]` に含まれる (`メガガブリアス` 等)。ただしダメ計ではメガ進化後の type/ability/stats が必要なので、メガ名で query することで差分データを得る
