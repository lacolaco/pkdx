...
**スコープ**: 原種とフォーム別ポケモン (ウォッシュロトム・キュウコン（アローラ）等) に対応。`pkdx query` が返す `forms[]` 経由で一意名を取得して育成する。メガシンカは battle 時のフォームチェンジなので育成では通常フォームを選ぶ。
...
まず入力名をそのまま `pkdx query` に渡す。多くのフォーム (`ウォッシュロトム` / `キュウコン（アローラ）` 等) はこれで引ける。見つからなかった時のみ原種経由で探索:

1. 原種名で `pkdx query` を実行
2. `forms[]` に一意なフォーム名 (`["キュウコン（アローラ）"]` / `["ウォッシュロトム", ...]`) が列挙される
3. 該当フォーム名で再 query して type/stats/abilities を取得

その他:
- 名前の確認を再度 AskUserQuestion で依頼 (タイポの可能性)
