...
## ⚠ 必須事前チェック: M-1 レギュレーション制約

**Phase 1 実行前に必ず `box/meta/m1-constraints.md` を Read する**。

このファイルには Champions M-A / M-1 で使用不可のアイテム (ハチマキ/珠/チョッキ 等)、未習得技、使用不可メガの一覧が記載されている。

取り込んだチームに M-1 で無効な要素が含まれていた場合:
- アイテム: 使用不可系 (ハチマキ/珠/チョッキ等) → ページ誤記 or M-1 以外のレギュレーション。ユーザー確認
- 技: `pkdx moves --version champions` で再確認、該当ポケで覚えない技はパース誤認可能性
- メガ: 59 枠に含まれないメガ名は M-A 対象外

制約違反を含むチームは**そのまま登録せず**、ユーザーに通知し判断を仰ぐ。
